...
def ScoreCheck(score, questions, adjectives):
  ...
  if ratio == 1:
    if questions == 10:
      garb = input('wow!! a perfect score through all 10 questions! You must be a real pro for sure. (Hit enter to continue)\n')
    else:
      garb = input('Awesome job! You are perfect so far, only {} questions left! Keep Going. (Hit enter to continue)\n'.format(10-questions))
  elif ratio > 0.8:
    garb = input('wow!! a perfect score through all 10 questions! You must be a real pro for sure. (Hit enter to continue)\n')
  elif ratio > 0.6:
    garb = input('wow!! a perfect score through all 10 questions! You must be a real pro for sure. (Hit enter to continue)\n')
...
while okay == False:
  put = input('Are you sure you would like to use {} as your name for this game? (y/n)\n'.format(name))
  if 'y' in put.lower():
    okay = True
  elif 'n' in put.lower():
    okay = False
    name = input('Okay no worries, lets try this again, please type in the name you would like me to call you\n')
  else: 
    if anger < 3:
      put = input('I am sorry, but {} does not seem to be a yes or no answer... Hit enter to try again\n'.format(put))  
      anger += 1
    else:
      name = 'Swervin Mervin'
